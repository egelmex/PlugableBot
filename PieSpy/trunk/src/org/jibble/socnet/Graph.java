...
public class Graph implements java.io.Serializable {
	 ...
	 * @param config
	 */
	public Graph(String label, Configuration config, PieSpy pieSpy) {
		_label = label;
		this.config = config;
		this.pieSpy = pieSpy;
	...
	public void infer(String nick, String message) {
		...
		for (int i = 0; i < _heuristics.size(); i++) {
			InferenceHeuristic heuristic = (InferenceHeuristic) _heuristics
					.get(i);
			changed = changed || heuristic.infer(nick, message);
		}
	...
	/**
	 * Returns the set of all Nodes that have emanating Edges. This therefore
	 * returns all Nodes that will be visible in the drawing.
	 * 
	 * @return
	 */
	...
	public BufferedImage drawImage(int width, int height, int borderSize,
		...
		g.drawString("Generated by " + pieSpy.bot.Nick() + " using "
				+ Configuration.VERSION, borderSize, height + borderSize * 2
				- 5 - 30);
		g
				.drawString(
	...
	public void writeGraph() {
		try {
			...
			oos.writeObject(Configuration.VERSION);
